...
export class WorkflowsService {
  ...
  async create(userId: string, dto: CreateWorkflowDto): Promise<Workflow> {
    const wf = this.workflowRepo.create({ userId, ...dto, graphData: dto.graphData ?? {} })
  ...
  async createRun(workflowId: string, userId: string, inputs?: Record<string, unknown>): Promise<WorkflowRun> {
    const run = this.runRepo.create({ workflowId, userId, inputs: inputs ?? null })
    return this.runRepo.save(run)
  }

  async updateRun(runId: string, patch: Partial<Pick<WorkflowRun, 'status' | 'temporalRunId' | 'outputs' | 'errorMessage' | 'elapsedMs' | 'totalTokens' | 'finishedAt'>>): Promise<void> {
    await this.runRepo.createQueryBuilder().update(WorkflowRun).set(patch as object).where('id = :id', { id: runId }).execute()
  }
