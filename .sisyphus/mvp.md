...
- 工作流 JSON 存在后端 `t_workflow.definition` 字段（JSONB）

#### 3.2 节点类型（MVP 仅 3 种）

| 节点类型 | 说明 |
|----------|------|
| 开始节点 | 工作流入口，接收用户输入（单个文本 `input` 字段）|
| LLM 节点 | 调用大语言模型，含 System Prompt + 引用变量 |
| 结束节点 | 工作流出口，展示输出内容 |
...
- 点击「保存」→ 将 Vue Flow 的 `nodes` + `edges` 序列化为 JSON → 调 `PUT /api/v1/workflows/:id`（字段 `definition`）
- 进入编辑器时，从 `GET /api/v1/workflows/:id` 加载 `definition`，反序列化还原画布
...
| GET | `/api/v1/workflows/:id` | 工作流详情（含 definition）|
| PUT | `/api/v1/workflows/:id` | 保存工作流（含 definition）|
